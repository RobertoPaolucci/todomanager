...
function normalizeReferenceValue(value?: string | null) {
...
function isAgencyChannel(channel?: Channel | null) {
  const type = normalizeText(channel?.type);
  return type === "agency";
}

function isAutoReferenceChannel(channel?: Channel | null) {
  return isDirectChannel(channel) || isAgencyChannel(channel);
}

function getExpectedReferencePrefix(channel?: Channel | null) {
...
function isAutomaticReferenceValue(value: string) {
  const normalized = normalizeReferenceValue(value);

  return (
    /^DIR-\d{6}$/i.test(normalized) ||
    /^AGY-\d{6}$/i.test(normalized) ||
    /^FMDQ-?\d+$/i.test(normalized)
  );
}
...
export default function BookingForm({
  ...
  const [nonPayingAdults, setNonPayingAdults] = useState(
    initialData?.non_paying_adults ?? 0
  );

  const [bookingReference, setBookingReference] = useState(
    initialData?.booking_reference ?? ""
  );
  const [autoGeneratedReference, setAutoGeneratedReference] = useState("");
  const [loadingAutoReference, setLoadingAutoReference] = useState(false);
  ...
  useEffect(() => {
    if (viewOnly) return;
    if (!channelId) return;
    if (!isAutoReferenceChannel(selectedChannel)) return;

    const expectedPrefix = getExpectedReferencePrefix(selectedChannel);
    const currentReference = normalizeReferenceValue(bookingReference);
    const currentAutoGeneratedReference =
      normalizeReferenceValue(autoGeneratedReference);

    const shouldGenerate =
      !currentReference ||
      currentReference === currentAutoGeneratedReference ||
      (isAutomaticReferenceValue(currentReference) &&
        !currentReference.startsWith(expectedPrefix));
    ...
    async function loadNextReference() {
      try {
        setLoadingAutoReference(true);

        const res = await fetch(
          `/api/bookings/next-direct-ref?channelId=${encodeURIComponent(
            channelId
          )}`,
          {
            cache: "no-store",
        ...
        const nextReference = normalizeReferenceValue(data?.reference);

        if (!cancelled && nextReference) {
          setBookingReference(nextReference);
          setAutoGeneratedReference(nextReference);
        }
      } catch {
        // nessun blocco del form
      } finally {
        if (!cancelled) {
          setLoadingAutoReference(false);
        }
      }
    }
  ...
  useEffect(() => {
    ...
    if (
      currentAutoGeneratedReference &&
      currentReference === currentAutoGeneratedReference
    ) {
      setBookingReference("");
    }

    if (autoGeneratedReference) {
      setAutoGeneratedReference("");
    }
  }, [selectedChannel, bookingReference, autoGeneratedReference, viewOnly]);
  ...
  function handleCancel() {
    const target = isEditing ? returnTo || "/prenotazioni" : "/prenotazioni";
    ...
    <form action={handleFormAction} className="grid gap-4 md:grid-cols-2">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="min-w-0">
            ...
            <input
              ...
              placeholder={
                isAutoReferenceChannel(selectedChannel)
            ...
            {!viewOnly &&
            isAutoReferenceChannel(selectedChannel) &&
            loadingAutoReference ? (
              <p className="mt-1 text-xs text-zinc-500">
                Generazione riferimento automatico...
              </p>
            ) : null}
          </div>
      ...
      <div className={sectionClass}>
        <div className="mb-4">
          ...
          <p className="mt-1 text-xs text-zinc-500 sm:text-sm">
            I paganti sono adulti + bambini. Gli infanti e guide/autisti non
            incidono sul prezzo, ma contano nei posti a tavola.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 sm:gap-3">
          ...
          <div className="min-w-0 rounded-2xl border border-amber-100 bg-amber-50/50 p-2.5">
            <label className="mb-1 block text-center text-[11px] font-bold uppercase text-amber-700">
              Guide / autisti
            </label>
            <input
              name="non_paying_adults"
              type="number"
              min="0"
              value={nonPayingAdults}
              onChange={(e) => setNonPayingAdults(Number(e.target.value))}
              disabled={viewOnly}
              className={nonPayingInputStyle}
            />
          </div>
        </div>
        ...
        <div className="mt-4 rounded-2xl bg-zinc-900 px-4 py-3 text-white">
          ...
          <div className="mt-1 flex items-center justify-between gap-3 text-sm">
            <span className="text-zinc-300">Guide / autisti non paganti</span>
            <span className="font-bold">{nonPayingAdults}</span>
          </div>
        </div>
      </div>
