...
function isAutomaticReferenceValue(value: string) {
  return /^DIR-\d{6}$/i.test(value) || /^AGY-\d{6}$/i.test(value);
}
...
export default function BookingForm({
  ...
  useEffect(() => {
    ...
    const currentReference = bookingReference.trim().toUpperCase();

    const shouldGenerate =
      !currentReference ||
      currentReference === autoGeneratedReference ||
    ...
    async function loadNextReference() {
      try {
        ...
        if (!cancelled && data?.reference) {
          ...
          setAutoGeneratedReference(data.reference);
        }
      } catch {
  ...
  useEffect(() => {
    ...
    if (autoGeneratedReference && bookingReference === autoGeneratedReference) {
      setBookingReference("");
    }
