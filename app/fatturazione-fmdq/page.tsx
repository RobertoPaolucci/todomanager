...
function getMonthBounds(month: string) {
  ...
  const lastDayDate = new Date(year, monthNumber, 0);

  const lastDay = `${year}-${String(monthNumber).padStart(2, "0")}-${String(
    lastDayDate.getDate()
  ).padStart(2, "0")}`;
...
export default async function FatturazioneFmdqPage({
   ...
   *
   * Un canale = una fattura del mese.
   */
   ...
   * CONTEGGIO FATTURE
   *
   * Ogni canale presente nel mese
   * corrisponde a una fattura.
   */
    ...
    >
      <div className="space-y-6">
        ...
        <SectionCard title="Periodo">
          ...
          <form
            method="GET"
            className="mt-5 flex flex-col gap-2 border-t border-zinc-100 pt-4 sm:flex-row sm:items-end"
          >
