...
export async function getCloudData() {
  const [mappingsRes, logsRes, expRes] = await Promise.all([
    supabase.from("import_mappings").select("*"),
  ...
  if (mappingsRes.data) {
    mappingsRes.data.forEach(row => { mappings[row.bokun_title] = row.experience_id; });
  }
  return { mappings, logs: logsRes.data || [], experiences: expRes.data || [] };
}

export async function saveMappingToCloud(title: string, experienceId: number | null) {
  if (experienceId) {
    await supabase.from("import_mappings").upsert({ bokun_title: title, experience_id: experienceId });
  } else {
    await supabase.from("import_mappings").delete().eq("bokun_title", title);
  }
}

export async function clearLogsFromCloud() {
  await supabase.from("import_logs").delete().neq("id", 0);
}
...
export async function importBokunBookings(rows: any[], mappings: Record<string, number>) {
  ...
  for (const row of rows) {
    try {
      const isGYG = row.hasOwnProperty("Booking Ref #") && row.hasOwnProperty("Product");
      let ref = "", rawTitle = "", excelStartDate = null, excelCreationDate = null;
      let customerName = "", email = null, phone = null, rawChannel = "Diretto";
      let isPaid = false, isCancelled = false, adults = 0, children = 0, infants = 0;
      let isDirectOrEbike = false;
      let fileTotalCustomer = 0, fileTotalNet = 0;
      ...
      if (isGYG) {
        ...
        customerName = `${row["Traveler's First Name"] || ""} ${row["Traveler's Last Name"] || ""}`.trim();
        email = row["Email"] || null;
        phone = row["Phone"] || null;
        rawChannel = "GetYourGuide";
        adults = Number(row["Adult"] || 0) + Number(row["Senior"] || 0) + Number(row["Youth"] || 0) + Number(row["EU Citizens (with ID)"] || 0);
        children = Number(row["Child"] || 0) + Number(row["Student (with ID)"] || 0);
        infants = Number(row["Infant"] || 0);
        isPaid = row["Reserve Now Pay Later Booking"] !== "Yes"; 
        fileTotalCustomer = parseFloat(String(row["Price"] || "0").replace(/[^\d.]/g, '')) || 0;
        fileTotalNet = parseFloat(String(row["Net Price"] || "0").replace(/[^\d.]/g, '')) || 0;
      } else {
        const cartCode = String(row["Cart confirmation code"] || "");
        const extRef = String(row["Ext. booking ref"] || row["External Booking Ref."] || row["External booking ref"] || "");
      ...
      if (!ref || ref === "undefined") ref = String(row["Product confirmation code"] || "");
      if (!ref) { results.skipped++; continue; }

      const experienceId = mappings[rawTitle];
      if (!experienceId) { results.skipped++; continue; }
      ...
      const finalPaymentStatus = (isPaid || new Date(date) < today) ? "paid" : "pending";

      const cleanStr = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, '');
      const cleanRawChannel = cleanStr(rawChannel);
      let channel = channels?.find(c => {
        const cleanC = cleanStr(c.name);
        return cleanC === cleanRawChannel || cleanC.includes(cleanRawChannel) || cleanRawChannel.includes(cleanC);
      });

      if (!channel && isDirectOrEbike) {
        channel = channels?.find(c => {
          const cleanC = cleanStr(c.name);
          return cleanC.includes("todo") || cleanC.includes("dirett") || cleanC.includes("sito");
        });
        if (channel) rawChannel = channel.name;
      }

      let fLord = 0, fNet = 0, fUnitL = 0, fUnitN = 0;
      const unitCost = exp?.supplier_unit_cost || 0;

      if (isGYG && fileTotalCustomer > 0) {
        fLord = fileTotalCustomer; fNet = fileTotalNet;
        fUnitL = pricing_pax > 0 ? fLord / pricing_pax : 0;
        fUnitN = pricing_pax > 0 ? fNet / pricing_pax : 0;
      } else {
        const { data: pR } = await supabase.from("experience_channel_prices").select("*")
          .eq("experience_id", experienceId).eq("channel_id", channel?.id || 0).maybeSingle();
        fUnitL = pR?.public_unit_price || 0;
        fUnitN = pR?.your_unit_price || 0;
        fLord = fUnitL * pricing_pax;
        fNet = fUnitN * pricing_pax;
      }
      ...
      const { error } = await supabase.from("bookings").insert({
        ...
        margin_total: fNet - (unitCost * pricing_pax),
        customer_payment_status: finalPaymentStatus,
        is_cancelled: isCancelled
      });

      if (error) {
        if (error.code === "23505") results.skipped++; else throw new Error(error.message);
      } else { results.imported++; }
    } catch (e: any) {
      const rowRef = row["Booking Ref #"] || row["Product confirmation code"] || "Sconosciuto";
      results.errors.push(`Rif: ${rowRef}: ${e.message}`);
    }
  }
  await supabase.from("import_logs").insert({ imported: results.imported, skipped: results.skipped, errors: results.errors.length });
  revalidatePath("/prenotazioni"); revalidatePath("/"); return results;
}