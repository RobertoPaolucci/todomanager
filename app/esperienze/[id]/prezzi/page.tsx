...
export default async function PrezziEsperienzaPage({ params }: PageProps) {
  ...
  const [
    ...
    supabaseServer
      ...
      .select(
        "id, channel_id, your_unit_price, your_child_unit_price, public_unit_price, public_child_unit_price, supplier_adult_unit_cost, supplier_child_unit_cost, currency, notes"
      )
  ...
  const baseSupplierAdultCost = Number(experience?.supplier_unit_cost || 0);
  ...
  return (
    <AppShell title="Prezzi canali" subtitle={experience?.name ?? ""}>
      <div className="w-full xl:w-[1250px] xl:max-w-[calc(100vw-340px)]">
        ...
        <SectionCard title="Prezzi per canale">
          <form action={saveExperienceChannelPrices} className="space-y-6">
            ...
            <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
              <strong>Costi fornitore:</strong> ora puoi impostare sia il costo
              fornitore adulto sia il costo fornitore bambino per ogni canale.
              Se lasci vuoto il costo adulto, Todo Manager userà il costo base
              della scheda esperienza:{" "}
                  ...
                  {(channels ?? []).map((channel) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-4 pr-4">
                          <input
                            name={`supplier_adult_unit_cost_${channel.id}`}
                            type="number"
                            step="0.01"
                            min="0"
                            defaultValue={valueForOptionalInput(
                              current?.supplier_adult_unit_cost
                            )}
                            className="w-32 rounded-xl border border-zinc-300 px-3 py-2 text-sm font-semibold outline-none transition focus:border-zinc-500"
                            placeholder={String(baseSupplierAdultCost)}
                          />
                          <div className="mt-1 text-[10px] text-zinc-400">
                            Vuoto = {formatCurrency(baseSupplierAdultCost)}
                          </div>
                        </td>
